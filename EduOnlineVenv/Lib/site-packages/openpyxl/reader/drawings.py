    ...
    for rel in drawing._blip_rels:
        ...
        if dep.Type == IMAGE_NS:
            try:
                ...
                msg = "The images {0} will be removed because it cannot be read".format(dep.target)
                warn(msg)
                continue
            if image.format.upper() == "WMF": # cannot save
                msg = "{0} images format is not supported so the images is being dropped".format(image.format)
                warn(msg)
                continue
