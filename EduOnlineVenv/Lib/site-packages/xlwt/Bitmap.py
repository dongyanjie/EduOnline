...
def _position_image(sheet, row_start, col_start, x1, y1, width, height):
    ...
    width  - Width of images frame
    height  - Height of images frame
    ...
    width = width + x1 - 1
    height = height + y1 - 1
    # Subtract the underlying cell widths to find the end cell of the images
    while (width >= _size_col(sheet, col_end)):
        width -= _size_col(sheet, col_end)
        col_end += 1
    # Subtract the underlying cell heights to find the end cell of the images
...
class ObjBmpRecord(BiffRecord):
    ...
    def __init__(self, row, col, sheet, im_data_bmp, x, y, scale_x, scale_y):
        # Scale the frame of the images.
        width = im_data_bmp.width * scale_x
        height = im_data_bmp.height * scale_y

        # Calculate the vertices of the images and write the OBJ record
...
def _process_bitmap_data(data):
    ...
    if (data[:2] != b"BM"):
        raise Exception("bitmap doesn't appear to to be a valid bitmap images.")
    ...
    data = data[8:]
    if (width > 0xFFFF):
        raise Exception("bitmap: largest images width supported is 65k.")
    if (height > 0xFFFF):
        raise Exception("bitmap: largest images height supported is 65k.")
    ...
    if (planes != 1):
        raise Exception("bitmap: only 1 plane supported in bitmap images.")
    # Read and remove the bitmap compression. Verify compression.
    compression = unpack("<L", data[:4])[0]
    data = data[4:]
    if (compression != 0):
        raise Exception("bitmap: compression not supported in bitmap images.")
...
class ImRawDataBmpRecord(BiffRecord):
    ...
    def __init__(self, data):
        """Insert a 24bit bitmap images in a worksheet. The main record required is
